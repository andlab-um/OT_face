...
file_prefix = "/Users/yuanchenwang/Library/Mobile Documents/com~apple~CloudDocs/Projects/fmri/MVPA-results/"
mask_filename = "/Users/yuanchenwang/Library/Mobile Documents/com~apple~CloudDocs/Projects/fmri/Analysis/mask_all.nii"
output_prefix = "/Users/yuanchenwang/Library/Mobile Documents/com~apple~CloudDocs/Projects/fmri/python/mvpa_perm/final/"
...
# load the actual accuracy, +25 as the baseline accuracy is 25% (1/4)
actual_OT = np.array(nib.load(file_prefix + "OT-25/actual/res_accuracy_minus_chance.img").get_fdata()) + 25
actual_PL = np.array(nib.load(file_prefix + "PL-25/actual/res_accuracy_minus_chance.img").get_fdata()) + 25

filtered_OT = actual_OT + 0
filtered_OT[filtered_OT<25] = 25
filtered_PL = actual_PL + 0
filtered_PL[filtered_PL<25] = 25
actual_diff = np.subtract(filtered_OT, filtered_PL)

# load all permutated images
for i in range(n):
    filepath = file_prefix + "OT-25/perm" + str(i+1) + "/res_accuracy_minus_chance.img"
    temp = nib.load(filepath).get_fdata()
    acc_OT[i] = np.array(temp) + 25

    filepath = file_prefix + "PL-25/perm" + str(i+1) + "/res_accuracy_minus_chance.img"
    temp = nib.load(filepath).get_fdata()
    acc_PL[i] = np.array(temp) + 25
...
def bootstrap(b):
    ...
    mean_OT[mean_OT<25] = 25
    boot_choice = acc_PL[np.random.choice(range(n), size=N_PL * 8)]
    mean_PL = np.mean(boot_choice, axis=0)
    mean_PL[mean_OT < 25] = 25
